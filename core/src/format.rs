...
impl RangeSettings {
    ...
    pub fn validate(self) -> crate::Result<Self> {
        if self.win_size > 30 {
            return Err(crate::DzipError::InvalidDz(format!(
                "WinSize {} exceeds 30",
                self.win_size
            )));
        }
        ...
        if self.flags & Self::USE_DZ_STATIC_TABLES != 0 {
            return Err(crate::DzipError::InvalidDz(
                "DZ static tables are rejected by dzip 1.1.3".to_string(),
            ));
        }
        ...
        if self.offset_tables == 0 {
            return Err(crate::DzipError::InvalidDz(
                "OffsetTables must not be zero".to_string(),
            ));
        }
        Ok(self)
    }
