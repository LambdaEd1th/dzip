...
- `zlib` starts from upstream `zlib-rs` 0.5.5 and restores the zlib 1.1.3 level-6
...
- `bzip` starts from `libbz2-rs-sys` 0.2.2, a Rust translation of
  libbzip2. `dzip` uses the reverse-engineered 1.0.3 parameters
  `blockSize100k=1`, `verbosity=0`, and `workFactor=30`.
- `lzma` is the repository's Rust implementation of the LZMA SDK 9.20
  path.
- `dz` implements the native range/LZ stream, archive-wide COMBUF
