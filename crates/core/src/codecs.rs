...
pub fn compress(input: &mut dyn Read, output: &mut dyn Write, flags: u16) -> Result<()> {
    ...
    if flags_enum.contains(ChunkFlags::LZMA) {
        // LZMA Compression
    ...
    if flags_enum.contains(ChunkFlags::ZLIB) {
        // ZLIB Compression
    ...
    if flags_enum.contains(ChunkFlags::BZIP) {
        // BZIP2 Compression
    ...
    // Default: Store (Copy without compression)
