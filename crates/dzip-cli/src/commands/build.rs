...
fn write_config(config: config::DclConfig, output_dir: &Path) -> Result<()> {
...
fn archive_builder(config: config::DclConfig) -> Result<ArchiveBuilder> {
    ...
    for entry in config.files {
        ...
        let (source, bytes) = read_source(&entry, &dcl_search_dirs)?;
